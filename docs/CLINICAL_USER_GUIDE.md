...
> 本手册面向医生在临床场景下使用 DoctorAgent 临床 AI 平台。所有示例输入/输出均为脱敏的合成数据。
...
DoctorAgent 是一个**合规优先、本地化部署**的临床 AI 智能体平台，部署在医疗机构自有基础设施内，PHI（受保护健康信息）原则上不离开可信边界。它为临床医生提供四类辅助能力：
...
医生在使用本平台前，必须理解并遵守以下四条原则：

1. **AI 不替代医生** —— 本平台输出为辅助参考，不构成诊断或医嘱；所有临床行动须经执业医师签发。
...
> ⚠️ **AI 误导必须上报**。出现以下情况须按医院不良事件流程上报，并通知平台管理员：
...
- 参考 `docs/CLINICAL_CAPABILITIES.md` 了解平台能力边界与「不做什么」清单。
...
> **最终提醒**：本平台一切输出均为辅助参考，**不替代医生临床判断，需经执业医师签发**。患者安全始终高于效率。
