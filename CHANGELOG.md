...
- **根因澄清**：环境升级到 fastapi 0.141.1 后，`include_router` 不再急拷贝路由到 `app.routes`，而是存入懒解析的 `_IncludedRouter` 包装器（首次请求时物化）。`test_route_auth_scan` 直接遍历 `app.routes` 只能看到 11 个包装器（表现为 "20 条路由"），结构断言误报。
- **修复**：新增 `_iter_api_routes()` 递归解包 `original_router`，同时兼容急拷贝（旧版）与懒引用（新版）两种布局；两个结构断言与 `/mcp/connect`、`/a2a/rpc` 钉住测试全部改走该解析器。
- **产品无影响**：HTTP 层行为验证通过——`/mcp/connect` 无 token 403、有 token 正常（TestClient 走完整请求路径，懒路由正常解析）。此前各版本的安全闸门（鉴权依赖）在两种语义下均生效。

### 验证

- 组合回归 **77 passed**（含此前因该问题失败的 route_auth_scan ×2）
...
核心审计发现的缺口：循环只有迭代数/工具次数预算，没有总时长闸——卡死的工具或极慢的 LLM 可以把一次运行钉死任意久。

- **`AgentConfig.max_wall_clock_seconds`**（默认 `None` 保持旧行为；设值如 `300` 生效）
- 到期后停止迭代，转入与"迭代耗尽"相同的收敛路径：让 LLM 基于已收集的观察给出最终答案，而不是静默丢弃
- 同步覆盖同步 `_react_loop` 与流式 `run_stream` 两条路径
- 计划/反思阶段此前已有各自超时（`_llm_timeout`），本预算补齐的是总闸

### 测试

- 新增 `tests/test_agent_wall_clock.py`（3 用例）：默认无限制、过期预算跳过迭代直接收敛（且不执行工具）、宽裕预算不影响正常路径 — **3 passed**
- 回归：test_agent_react_loop **50 passed** + test_agent **39 passed**
...
38 个侧边栏入口中约 36 个是"GET 端点 → JSON 拍平 → 表格"的 API 浏览器，把企业级功能矩阵呈现得比实际成熟度更高，透支用户信任。

- **侧边栏收敛为 8 个真实视图**（临床对话 / 临床工作台 / Vault / PHI 去标识化 / 审计链 / 模型连接 / 系统状态 / 设置）；其余视图仍可通过 URL 直达，但不再作为产品功能宣传。
- **路由懒加载**：38 个视图全部改为动态 import，按视图分块。主 bundle 从 **216KB → 107KB**，ChatView（marked/DOMPurify）独立成 71KB chunk。
- **移除死依赖**：`chart.js`、`highlight.js` 在源码中零引用（仅旧版 CDN vendor 方案使用），从 package.json 移除。
- 修正过期注释（"29 console views"）。
- dist 构建产物随本次变更重新生成提交（仓库当前策略保留预构建 dist；后续应改为 CI 发布）。
...
修正 README（中/英）中超出实现程度的声明，避免医疗机构在规划期被误导：

| 条目 | 之前 | 之后 |
|---|---|---|
| HIPAA Safe Harbor | "已实现" | **部分实现**——正则/启发式脱敏，姓名识别未达法律召回率标准，生产建议叠加 NER + 人工抽检 |
| 审计链防篡改 | （未列边界） | 新增行：HKDF 派生密钥；root + 主密钥口令仍可重签，对抗需远程 append-only 副本 |
| RBAC 角色控制 | （未列） | 新增行：v0.3.7 起管理端点已接线，其余端点仍为单一 token 全权 |
| FAQ 患者数据保护 | "HIPAA Safe Harbor" 直述 | 注明为"基于 Safe Harbor 类别的正则启发式实现" |

> README.ja.md 为旧版内容（仍含已移除的 PyQt6 描述），整体重写另开 PR。
