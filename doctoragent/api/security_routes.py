...
def get_router() -> APIRouter:
    ...
    async def security_overview(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def add_threat_case(
        ...
        try:
            return _get(request, "threat_service").store.add_threat_case(
                payload.get("name", ""), payload.get("threat_type", "prompt_injection"),
                payload.get("attack_vector", ""), severity=payload.get("severity", "medium"),
    ...
    async def scan_input(
        ...
        return svc.scan_input(payload.get("text", ""), user_id=payload.get("user_id", ""),
    ...
    async def list_redteam(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def interop_overview(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
        return _get(request, "interop_service").overview()

    @router.get("/interop/directory", summary="External agent directory")
    async def interop_directory(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def list_policies(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def add_policy(
        ...
        p = _get(request, "interop_service").add_policy(
            payload.get("name", "policy"), allow_agents=payload.get("allow_agents"),
    ...
    async def check_access(
        ...
        return _get(request, "interop_service").check_access(
            payload.get("agent", ""), payload.get("action", ""),
    ...
    async def dr_metrics(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
        return _get(request, "disaster_service").metrics()

    @router.get("/dr/backups", summary="List backup jobs")
    async def list_backups(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def add_backup(
        ...
        return _get(request, "disaster_service").register_backup_job(
            payload.get("name", ""), payload.get("scope", "database"),
    ...
    async def run_backup(job_id: str, request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
        try:
    ...
    async def list_dr_plans(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
    ...
    async def create_dr_plan(
        ...
        return _get(request, "disaster_service").create_dr_plan(
            payload.get("name", ""), int(payload.get("rto_target_s", 60)),
            int(payload.get("rpo_target_s", 300)), tier=int(payload.get("tier", 3)),
    ...
    async def run_drill(
        ...
        return _get(request, "disaster_service").run_drill(
            payload.get("name", "drill"), payload.get("plan_id", ""),
            scenario=payload.get("scenario", "failover"),
        )

    @router.get("/dr/drills", summary="List drill history")
    async def list_drills(request: Request, _auth: Any = Depends(_auth_dependency)) -> dict[str, Any]:
